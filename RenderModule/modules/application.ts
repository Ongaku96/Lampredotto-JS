...
import { DataCollection, ReactivityOptions, Settings, TemplateOptions, iEvent } from "./types.js";
...
class Application {
    ...
    public name: string = "";//id of application
    public context: DataCollection = {};//data context
    public vdom: vNode | undefined = undefined;//virtual DOM
    public settings: Settings = new Settings({ debug: true });//Application settings
    ...
    private _reactivity: ReactivityOptions = {
        handler: this.handler
    };
    public get reactivity() { return this._reactivity; } //Proxy rules set
    ...
    async build(options: TemplateOptions = {}) {
        try {
    ...
    private applySettings() {
        ...
        switch (this.settings.interface?.animation) {
            case "slower": document.documentElement.style.setProperty(`--global-animation`, "5s"); break;
            case "slow": document.documentElement.style.setProperty(`--global-animation`, "4s"); break;
            case "fast": document.documentElement.style.setProperty(`--global-animation`, "3s"); break;
            case "faster": document.documentElement.style.setProperty(`--global-animation`, "1s"); break;
            case "unset": document.documentElement.style.setProperty(`--global-animation`, "0s"); break;
        }
        if (this.settings.interface?.radius) document.documentElement.style.setProperty(`--global-radius`, this.settings.interface.radius);
        if (this.settings.interface?.animation) document.documentElement.style.setProperty(`--global-animation`, this.settings.interface.animation);
        if (this.settings.interface?.font) document.documentElement.style.setProperty(`--global-font`, this.settings.interface.font);
        function updateGlobalStyle(name: string, hex: string) {
    ...
    updateSettings(settings: Settings) {
        if (settings.interface) this.settings.interface = settings.interface;
        this.applySettings();
    }
    //#endregion
    ...
    async buildContext(dataset: DataCollection, actions?: any, getters?: any): Promise<Application> {
        return Support.elaborateContext(this.context, dataset, this.reactivity, actions, getters).then((output) => {
            if (valueIsNotReactive(output)) output = react(output, this.reactivity);
            let app = this;
    ...
    defaultEvents() {
        this.onProgress((state: string, message: string) => {
