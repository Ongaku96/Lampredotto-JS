    ...
    render(node: vNode) {
        try {
            ...
            function isActive(node: vNode) {
                for (const element of node.reference) {
                    if ((<HTMLElement>element).querySelector("input:focus")) return true;
                }
                return false;
            }
        } catch (ex) {
...
class cOn extends Command {
    ...
    accept(options: CommandOptions) {
        this.event = {
            ...
            action: function (evt: Event, ...args: any[]) { return elaborateContent(options.value, this, evt, [], true, ...args); }
        };
        this._handler.trigger(Collection.node_event.setup, options);
    }
